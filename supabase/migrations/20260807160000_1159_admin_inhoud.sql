-- #1159 Beheerder mag overal aan — de leesfuncties en de overdracht van
-- eigenaarschap waarop de edge function `admin-content` draait. Spiegel van
-- supabase/schemas/functions/40_admin_inhoud.sql en van de twee nieuwe kolommen
-- in supabase/schemas/tables/26_app_admins.sql; zie die bestanden voor de
-- volledige motivatie.
--
...
-- 20260805120000_1036_adminpaneel.sql). Deze migratie voegt uitsluitend nieuwe
-- objecten toe plus twee nullable kolommen, dus ze is één-op-één na te lezen
-- naast de schemabestanden.

-- 1. Auditspoor: waar ging de actie over? ------------------------------------
--
-- Tot nu toe kende het logboek alleen een doel-gebruiker. Een actie op een
-- match of een groep past daar niet in. Nullable, dus de bestaande rijen uit
-- #1036 blijven precies zoals ze zijn.
alter table public.admin_audit_log
  add column target_type text
    check (target_type is null or target_type in ('match', 'group', 'poll')),
  add column target_id uuid;

create index admin_audit_log_object_idx
  on public.admin_audit_log (target_type, target_id, created_at desc);

-- 2. Alle matches, over alle groepen heen ------------------------------------
--
