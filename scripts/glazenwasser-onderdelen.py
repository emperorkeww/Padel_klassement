...
* ``gw-ring.webp``: frame, badges, trekker, emmer, doek, spray en spons;
* ``gw-glas.webp``: een rustige, naadloze natglastegel voor het kaartvlak;
* ``glazenwasser-master.webp`` en ``glazenwasser-front-mask.webp`` voor de
...
def bouw() -> dict[str, dict]:
    ...
    props = np.maximum.reduce([top_badge, squeegee, bucket, sponge, bottom_badge, top_foam])
    ...
    ring_alpha = np.clip(np.maximum(frame, props), 0, 1)
    ring_alpha *= np.where(buiten > 0.55, 1.0, buiten_key)
    ...
    ring_k = ring.resize((kb, kh), Image.Resampling.LANCZOS)
    master.alpha_composite(achter_k, (kx, ky))
    master.alpha_composite(ring_k, (kx, ky))
    ...
    if "--preview" in sys.argv:
        ...
        voorbeeld.alpha_composite(ring)
