...
// De tabbalk in reis-volgorde. De id's zijn de URL-keys (?tab=…) en staan in
// pushberichten, feed-links en edge functions — labels mogen wisselen (#673),
// keys niet.
const TABS: { id: View; label: string }[] = [
  { id: "plannen", label: "Plannen" },
  { id: "rondes", label: "Vandaag" },
  { id: "spelen", label: "Teams" },
  { id: "matches", label: "Historie" },
  { id: "stand", label: "Stand" },
  { id: "leden", label: "Leden" },
];

const TABPANEL_ID = "group-tabpanel";
...
export function GroupDetail() {
  ...
  // Voor de pijltjes-navigatie: focus verplaatsen naar de nieuwe tabknop.
  ...
  // Tellers per tab; 0 = geen teller tonen.
  const tabCounts: Record<View, number> = {
    plannen: 0,
    rondes: rounds.length,
    spelen: 0,
    matches: completedMatches.length,
    stand: 0,
    leden: memberList.length,
  };
  // Pijltjes-navigatie binnen de tablist (WAI-ARIA tabs pattern): selectie
  // volgt de focus, zoals bij de meeste tabbalken.
  ...
  return (
    <div>
      <header className="page-head">
        ...
        {/* Het ledenaantal staat al als teller op de Leden-tab (#674). */}
        {isOwner && <p className="page-subtitle">jij bent eigenaar</p>}
      </header>
      ...
      {/* Tabs in reis-volgorde (#106): plannen → spelen → stand.
          ...
          Echte tab-semantiek (#674): rol, aria-selected, pijltjes en de
          teller in de toegankelijke naam ("Leden, 6"). De balk scrollt
          horizontaal via .tabs--page — zes labels passen niet op 375px. */}
      <div
        className="tabs tabs--page"
        role="tablist"
        aria-label="Groepsonderdelen"
        ...
        {TABS.map((t, i) => {
          ...
          return (
            <button
              ...
              // Eén tabstop voor de hele balk (roving tabindex): binnen de
              // balk navigeer je met de pijltjes, niet met Tab.
              tabIndex={active ? 0 : -1}
              aria-label={count ? `${t.label}, ${count}` : undefined}
      ...
      <div id={TABPANEL_ID} role="tabpanel" aria-labelledby={`grouptab-${view}`}>
        {view === "rondes" && (
