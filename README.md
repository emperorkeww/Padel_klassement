...
| `club-search` | Aangeroepen door de Cloudflare Worker (`env.CLUB_SEARCH_EGRESS`). Egress-hop voor het zoeken naar clubs: Playtomic heeft geen zoek-API meer, dus deze functie leest de publieke zoekpagina (`playtomic.com/search`, RSC-payload) en geeft de geparste Belgische treffers terug (#391). Zelfde WAF-reden als `playtomic-availability`. Deployen met `--no-verify-jwt`. |
